$(document).ready(function () {
   ...
   function header_fixed_menu(){
       $('.header-big-menu').css('top', $('.header-wow-sticky-container').outerHeight()+50 );
   }
   ...
   $(document).on('click','.sticky-search-geo-list .item-city, .sticky-search-geo-results .item-city', function () {      
      ...
      $.ajax(
         ...
         type: "POST",
         url: url_path + "systems/ajax/geo.php",
         data: "city_id="+$(this).attr("id-city")+"&region_id="+$(this).attr("id-region")+"&country_id="+$(this).attr("id-country")+"&action=change-city",
         dataType: "html",
         cache: false,
         success: function (data) { 
            location.reload();
