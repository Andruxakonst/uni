...
function initChat(id_ad=0){

 $(".init-chat-body").html('<div class="chat-dialog-spinner"><div class="spinner-border text-primary" role="status"></div></div>');

 $.ajax({type: "POST",url: url_path + "systems/ajax/chat.php",data: "id_ad="+id_ad+"&action=init",dataType: "html",cache: false,success: function (data) { 
    if(data){
...
$(document).on('click','.chat-dialog-text-send', function (e) { 
    if(status_send == true){
        status_send = false;
        sendChat();
    }  
...
$(document).on('input','.chat-dialog-text', function (e) { 
    if ($(this).val()!='' || $(".attach-files-preview").length) {
        $('.chat-dialog-text-send').show();
        $('.chat-dialog-audio-start').hide();
    }else{
        $('.chat-dialog-text-send').hide();
        $('.chat-dialog-audio-start').show();
    }
});
...
$(document).on("click", ".chat-dialog-attach-delete", function(e) {
    ...
    if(!$(".attach-files-preview").length && $('.chat-dialog-text').val()==''){
       $('.chat-dialog-text-send').hide();
       $('.chat-dialog-audio-start').show();
    }
...
function updateCount(){
    ...
    if(statusUpdateCount){
       $.ajax({type: "POST",url: url_path + "systems/ajax/chat.php",data: "id="+$('.module-chat-users div.active').data('id')+"&action=update_count_message",dataType: "json",cache: false,success: function (data) { 
