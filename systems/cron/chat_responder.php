...
if($getChatResponder){
    ...
    if(count($getActiveUsers)){
       foreach ($getActiveUsers as $user) {
           $Profile->sendChat(array( "support" => 1, "id_responder" => $getChatResponder["chat_responders_id"], "id_hash" => md5('support'.$user['clients_id']), "text" => urldecode($getChatResponder['chat_responders_text']), "user_from" => 0, "user_to" => $user['clients_id']));
       }
    }
